/*
  # Complete Database Schema Migration - Safe Policy Management
  
  This migration safely handles existing policies and constraints while ensuring
  all required database enhancements are applied correctly.
  
  ## Changes Applied:
  1. Security - Enable RLS and create comprehensive policies
  2. Data Validation - Add check constraints for data integrity  
  3. Performance - Create indexes for optimized queries
  4. Utility Functions - Add helper functions for API limits and debugging
  5. Triggers - Add updated_at timestamp management
*/
...
SELECT add_constraint_if_not_exists(
  'manual_activities',
  'manual_activities_activity_type_check',
...
SELECT add_constraint_if_not_exists(
  'api_usage_tracking',
  'api_usage_service_check',
  'CHECK (api_service = ''tavus''::text)'
);

SELECT add_constraint_if_not_exists(
  'api_usage_tracking',
  'api_usage_request_type_check',
...
-- =====================================================
-- 9. VERIFICATION (Optional - for debugging)
-- =====================================================
