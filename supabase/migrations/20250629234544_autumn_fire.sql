...
-- Resolves column ambiguity and function conflicts
-- =====================================================
...
DROP FUNCTION IF EXISTS get_foreign_key_constraints(TEXT);
...
-- =====================================================
-- 5. SAFELY ADD DATA VALIDATION CONSTRAINTS
-- =====================================================
...
-- =====================================================
-- 7. CREATE UTILITY FUNCTIONS WITH UNIQUE NAMES
-- =====================================================
...
CREATE OR REPLACE FUNCTION get_table_columns_info(input_table_name TEXT)
...
CREATE OR REPLACE FUNCTION get_foreign_key_info(input_table_name TEXT)
...
BEGIN
  ...
  WHERE tc.constraint_type = 'FOREIGN KEY'
    AND tc.table_name = input_table_name
    AND tc.table_schema = 'public';
END;
$$;

-- Function to check if column exists
CREATE OR REPLACE FUNCTION check_column_exists_in_table(input_table_name TEXT, input_column_name TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(calories_burned), 0)::INTEGER as total_calories,
    COALESCE(
      jsonb_object_agg(
        activity_type, 
        activity_count
  ...
  FROM (
    ...
    WHERE user_id = check_user_id
    GROUP BY activity_type, distance_km, duration_seconds, calories_burned
  ) stats;
END;
$$;
...
-- =====================================================
-- 10. VERIFICATION AND SUCCESS NOTICES
-- =====================================================
